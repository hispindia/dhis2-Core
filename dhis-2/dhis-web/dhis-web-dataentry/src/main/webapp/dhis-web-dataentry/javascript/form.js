...
$(document).ready(function () {
  ...
  $("#orgUnitTree").one("ouwtLoaded", function (event, ids, names) {
    ...
    $.when(
      dhis2.de.getMultiOrgUnitSetting(),
...
dhis2.de.manageOfflineData = function () {
  if (dhis2.de.storageManager.hasLocalData()) {
    var message =
      ...
      ' <button id="sync_button" type="button">' +
      i18n_sync_now +
      "</button>";
...
dhis2.de.shouldFetchDataSets = function (ids) {
  ...
  if (
    !$.isArray(ids) ||
    ids.length === 0 ||
    (ids.length > 0 && dhis2.de.fetchedDataSets[ids[0]])
  ) {
    return false;
  }
  ...
  if (
    $.isArray(c) &&
    c.length > 0 &&
    dhis2.de.organisationUnitAssociationSetMap[c[0]]
  ) {
    return false;
  }
...
dhis2.de.getMultiOrgUnitSetting = function () {
  return $.ajax({
    ...
    success: function (data) {
      var isMultiOrgUnit =
        data && data.multiOrganisationUnitForms
          ? data.multiOrganisationUnitForms
          : false;
...
dhis2.de.loadDataSetAssociations = function () {
  ...
  $.ajax({
    ...
    dataType: "json",
    success: function (json) {
      sessionStorage[dhis2.de.cst.dataSetAssociations] = JSON.stringify(
        json.dataSetAssociations
      );
    },
    complete: function () {
      var metaData = JSON.parse(
        sessionStorage[dhis2.de.cst.dataSetAssociations]
      );
      dhis2.de.dataSetAssociationSets = metaData.dataSetAssociationSets;
      dhis2.de.organisationUnitAssociationSetMap =
        metaData.organisationUnitAssociationSetMap;
      def.resolve();
    },
  });
...
dhis2.de.uploadLocalData = function () {
  ...
  var completeDataSetsArray = completeDataSets
    ? Object.keys(completeDataSets)
    : [];
  ...
  function pushCompleteDataSets(array) {
    ...
    console.log(
      "Uploaded complete data set: " + key + ", with value: " + value
    );
      ...
      error: function (jqXHR, textStatus, errorThrown) {
        if (409 === xhr.status || 500 === xhr.status) {
          ...
          var message =
            ...
            i18n_sync_now +
            "</button>" +
            ' <button id="discard_button" type="button">' +
            i18n_discard +
            "</button>";
  ...
  (function pushDataValues(array) {
    ...
    $.ajax({
      ...
      error: function (xhr, textStatus, errorThrown) {
        if (403 == xhr.status || 409 === xhr.status || 500 === xhr.status) {
          // Invalid value or locked
          // Ignore value for now TODO needs better handling for locking
  ...
  if (!dhis2.de.multiOrganisationUnit) {
    dhis2.de.storageManager.formExists(dataSetId).done(function (value) {
      if (value) {
        ...
        dhis2.de.storageManager
          .formExistsRemotely(dataSetId)
          .done(function (value) {
    ...
    $("#contentDiv").load(
      "loadForm.action",
      {
        ...
        multiOrganisationUnit: dhis2.de.multiOrganisationUnit
          ? dhis2.de.getCurrentOrganisationUnit()
          : "",
      },
      function () {
        if (!dhis2.de.multiOrganisationUnit) {
...
dhis2.de.enableSectionFilter = function () {
  ...
  if ($sectionHeaders.size() > 1) {
    $("#filterDataSetSection").append(
      "<option value='all'>" + i18n_show_all_sections + "</option>"
    );

    $sectionHeaders.each(function (idx, value) {
      $("#filterDataSetSection").append(
        "<option value='" + idx + "'>" + value.innerHTML + "</option>"
      );
    });
...
function refreshZebraStripes($tbody) {
  $tbody
    .find("tr:not([colspan]):visible:even")
    .find("td:first-child")
    .removeClass("reg alt")
    .addClass("alt");
  $tbody
    .find("tr:not([colspan]):visible:odd")
    .find("td:first-child")
    .removeClass("reg alt")
    .addClass("reg");
}
...
function getDataElementType(dataElementId) {
  ...
  console.log(
    "Data element not present in data set, falling back to default type: " +
      dataElementId
  );
...
function getDataElementName(dataElementId) {
  ...
  console.log(
    "Data element not present in form, falling back to default name: " +
      dataElementId
  );
...
function getOptionComboName(optionComboId) {
  ...
  console.log(
    "Category option combo not present in form, falling back to default name: " +
      optionComboId
  );
  return dhis2.de.cst.defaultName;
}
  ...
  dhis2.de.getOrFetchDataSetList().then(function (data) {
    ...
    if (children) {
      ...
      if (childrenDataSets && childrenDataSets.length > 0) {
        $("#selectedDataSetId").append(
          '<optgroup label="' + i18n_childrens_forms + '">'
        );
        ...
        $.safeEach(childrenDataSets, function (idx, item) {
          ...
          $("<option />")
            .attr("data-multiorg", true)
            .attr("value", item.id)
            .html(item.name)
            .appendTo("#selectedDataSetId");
        });
    ...
    if (!dhis2.de.multiOrganisationUnit && dataSetValid && dataSetId) {
      ...
      dhis2.de.multiOrganisationUnit &&
      multiDataSetValid &&
      dataSetId
    ) {
      $("#selectedDataSetId").val(dataSetId); // Restore selected data set
...
dhis2.de.fetchDataSets = function (ou) {
  var def = $.Deferred();
  var fieldsParam = encodeURIComponent(
    "id,dataSets[id],children[id,dataSets[id]]"
  );
...
dhis2.de._updateDataSets = function (ou) {
  ...
  dhis2.de.dataSetAssociationSets[
    Object.keys(dhis2.de.dataSetAssociationSets).length
  ] = dataSets;
  dhis2.de.organisationUnitAssociationSetMap[ou.id] =
    Object.keys(dhis2.de.dataSetAssociationSets).length - 1;
};
...
function getSortedDataSetList(orgUnit) {
  var associationSet =
    orgUnit !== undefined
      ? dhis2.de.organisationUnitAssociationSetMap[orgUnit]
      : dhis2.de.organisationUnitAssociationSetMap[
          dhis2.de.getCurrentOrganisationUnit()
        ];
...
function getSortedDataSetListForOrgUnits(orgUnits) {
  ...
  $.safeEach(dataSetList, function (idx, item) {
    ...
    if (
      !found &&
      (formType == dhis2.de.cst.formTypeSection ||
        formType == dhis2.de.cst.formTypeDefault)
    ) {
...
function dataSetSelected() {
  var previousDataSetValid =
    dhis2.de.currentDataSetId && dhis2.de.currentDataSetId != -1;
  var previousDataSet = !!previousDataSetValid
    ? dhis2.de.dataSets[dhis2.de.currentDataSetId]
    : undefined;
  var previousPeriodType = previousDataSet
    ? previousDataSet.periodType
    : undefined;
  var previousOpenFuturePeriods = previousDataSet
    ? previousDataSet.openFuturePeriods
    : 0;

  dhis2.de.currentDataSetId = $("#selectedDataSetId").val();

  var serverTimeDelta = dhis2.de.storageManager.getServerTimeDelta() || 0;
  dhis2.de.blackListedPeriods = dhis2.de.dataSets[
    dhis2.de.currentDataSetId
  ].dataInputPeriods
    .filter(function (dip) {
      return (
        (dip.openingDate != "" &&
          new Date(dip.openingDate) > Date.now() + serverTimeDelta) ||
        (dip.closingDate != "" &&
          new Date(dip.closingDate) < Date.now() + serverTimeDelta)
      );
    })
    .map(function (dip) {
      return dip.period.isoPeriod;
    });
  ...
  if (dhis2.de.currentDataSetId && dhis2.de.currentDataSetId !== -1) {
    ...
    var openFuturePeriods =
      dhis2.de.dataSets[dhis2.de.currentDataSetId].openFuturePeriods;

    var previousSelectionValid = !!(
      periodType == previousPeriodType &&
      openFuturePeriods == previousOpenFuturePeriods
    );

    dhis2.de.currentCategories = dhis2.de.getCategories(
      dhis2.de.currentDataSetId
    );

    dhis2.de.setAttributesMarkup();

    dhis2.de.multiOrganisationUnit = !!$("#selectedDataSetId :selected").data(
      "multiorg"
    );
...
function displayPeriods() {
  ...
  var periods = dhis2.period.generator.generateReversedPeriods(
    periodType,
    dhis2.de.currentPeriodOffset
  );
  ...
  if (periods.length > 0) {
    ...
    addOptionById(
      "selectedPeriodId",
      "",
      i18n_no_periods_click_prev_year_button
    );
  }
...
dhis2.de.getCategories = function (dataSetId) {
  ...
  if (
    ...
    dhis2.de.defaultCategoryCombo === dataSet.categoryCombo
  ) {
    return null;
  }
...
dhis2.de.optionValidWithinPeriod = function (option, period) {
  ...
  if (option.start) {
    optionStartDate = dhis2.period.calendar.parseDate(
      dhis2.period.format,
      option.start
    );
  }
  ...
  if (option.end) {
    optionEndDate = dhis2.period.calendar.parseDate(
      dhis2.period.format,
      option.end
    );
    var ds = dhis2.de.dataSets[dhis2.de.currentDataSetId];
    if (ds.openPeriodsAfterCoEndDate) {
      optionEndDate = dhis2.period.generator.datePlusPeriods(
        ds.periodType,
        optionEndDate,
        parseInt(ds.openPeriodsAfterCoEndDate)
      );
    }
  }

  var periodStartDate = dhis2.period.calendar.parseDate(
    dhis2.period.format,
    dhis2.de.periodChoices[period].startDate
  );
  var periodEndDate = dhis2.period.calendar.parseDate(
    dhis2.period.format,
    dhis2.de.periodChoices[period].endDate
  );

  return (
    (!optionStartDate || optionStartDate <= periodEndDate) &&
...
dhis2.de.getAttributesMarkup = function () {
  ...
  if (
    ...
    dhis2.de.currentCategories.length == 0 ||
    !period
  ) {
    return html;
  }
  ...
  $.safeEach(dhis2.de.currentCategories, function (idx, category) {
    ...
    html +=
      '<select id="category-' +
      category.id +
      '" class="selectionBoxSelect" onchange="dhis2.de.attributeSelected(\'' +
      category.id +
      "')\">";
    ...
    $.safeEach(category.options, function (idx, option) {
      if (
        dhis2.de.optionValidWithinPeriod(option, period) &&
        dhis2.de.optionValidForSelectedOrgUnit(option)
      ) {
        var selected =
          $.inArray(option.id, options) != -1 || category.options.length == 1
            ? " selected"
            : "";
        html +=
          '<option value="' +
...
function clearFileEntryFields() {
  ...
  $fields
    .find(".upload-field")
    .css("background-color", dhis2.de.cst.colorWhite);
...
function getAndInsertDataValues() {
  ...
  $(".entryfield")
    .css("background-color", dhis2.de.cst.colorWhite)
    .css("border", "1px solid " + dhis2.de.cst.colorBorder);
  $(".entryselect")
    .css("background-color", dhis2.de.cst.colorWhite)
    .css("border", "1px solid " + dhis2.de.cst.colorBorder);
  $(".indicator")
    .css("background-color", dhis2.de.cst.colorLightGrey)
    .css("border", "1px solid " + dhis2.de.cst.colorBorder);
  ...
  $(".entryfield")
    .filter(":disabled")
    .css("background-color", dhis2.de.cst.colorGrey);
...
function insertDataValues(json) {
  ...
  if (dataSet && dataSet.expiryDays > 0) {
    var serverTimeDelta = dhis2.de.storageManager.getServerTimeDelta() || 0;
    var maxDate = moment(period.endDate, dhis2.period.format.toUpperCase()).add(
      parseInt(dataSet.expiryDays),
      "day"
    );
  ...
  periodLocked =
    periodLocked && dhis2.de.lockExceptions.indexOf(lockExceptionId) == -1;

  if (
    json.locked !== "OPEN" ||
    dhis2.de.blackListedPeriods.indexOf(period.iso) > -1 ||
    periodLocked
  ) {
    dhis2.de.lockForm();
  ...
  if (!dhis2.de.multiOrganisationUnit) {
    if (period) {
      if (
        dhis2.de.validateOrgUnitOpening(
          organisationUnits[dhis2.de.getCurrentOrganisationUnit()],
          period
        )
      ) {
        dhis2.de.lockForm();
  ...
  $.safeEach(json.dataValues, function (i, value) {
    ...
    if ($(fieldId).length > 0) {
      ...
      if (
        "true" == value.val &&
        (entryField.attr("name") == "entrytrueonly" ||
          entryField.hasClass("entrytrueonly"))
      ) {
        $(fieldId).prop("checked", true);
      } else if (
        entryField.attr("name") == "entryoptionset" ||
        entryField.hasClass("entryoptionset")
      ) {
...
function valueFocus(e) {
  ...
  if (dhis2.de.multiOrganisationUnit) {
    organisationUnitName = organisationUnitList.filter(
      (ou) => ou.uid === dhis2.de.getCurrentOrganisationUnit()
    )[0].n;
  } else {
    organisationUnitName =
      organisationUnits[dhis2.de.getCurrentOrganisationUnit()].n;
  }
...
function keyPress(event, field) {
  ...
  var focusField =
    key == 13 || key == 40
      ? getNextEntryField(field)
      : key == 38
      ? getPreviousEntryField(field)
      : false;
...
function handleDataSetCompletenessResponse(data) {
  var html =
    "<h3>" +
    i18n_dataset_completeness_error +
    ' &nbsp;<img src="../images/warning_small.png"></h3>';
 ...
 */
dhis2.de.validate = function (
  completeUncomplete,
  ignoreValidationSuccess,
  successCallback
) {
  ...
  var compulsoryFieldsCompleteOnly =
    dhis2.de.dataSets[dhis2.de.currentDataSetId].compulsoryFieldsCompleteOnly;
  ...
  if (!compulsoryCombinationsValid || !compulsoryDataElementsValid) {
    if (!compulsoryDataElementsValid && !compulsoryFieldsCompleteOnly) {
      ...
      var html =
        ...
        ' &nbsp;<img src="../images/warning_small.png"></h3>' +
        '<p class="bold">' +
        i18n_missing_compulsory_dataelements +
        "</p>";
  ...
  var successHtml =
    "<h3>" +
    i18n_validation_result +
    ' &nbsp;<img src="../images/success_small.png"></h3>' +
    '<p class="bold">' +
    i18n_successful_validation +
    "</p>";

  var validCompleteOnly =
    dhis2.de.dataSets[dhis2.de.currentDataSetId].validCompleteOnly;
  ...
  $("#validationDiv").load(
    ...
    function (response, status, xhr) {
      ...
      if (success) {
        $(document).trigger(
          dhis2.de.event.validationSucces,
          dhis2.de.currentDataSetId
        );
      } else {
        $(document).trigger(
...
dhis2.de.validateCompulsoryCombinations = function () {
  var fieldCombinationRequired =
    dhis2.de.dataSets[dhis2.de.currentDataSetId].fieldCombinationRequired;

  if (fieldCombinationRequired) {
    var violations = false;

    $(".entryfield")
      .add('[name="entryselect"]')
      .each(function (i) {
        if ($(this).prop("disabled")) {
          return;
        }
        ...
        if (hasValue) {
          $selector = $(
            '[name="entryfield"][id^="' + dataElementId + '-"]'
...
dhis2.de.validateOrgUnitOpening = function (organisationUnit, period) {
  ...
  if (organisationUnit.odate) {
    odate = dhis2.period.calendar.fromJD(
      iso8601.parseDate("yyyy-mm-dd", organisationUnit.odate).toJD()
    );
  }

  if (organisationUnit.cdate) {
    cdate = dhis2.period.calendar.fromJD(
      iso8601.parseDate("yyyy-mm-dd", organisationUnit.cdate).toJD()
    );
  }

  var startDate = dhis2.period.calendar.parseDate(
    dhis2.period.format,
    period.startDate
  );
  var endDate = dhis2.period.calendar.parseDate(
    dhis2.period.format,
    period.endDate
  );
...
function StorageManager() {
    ...
    DAO.store.get("forms", dataSetId).done(function (form) {
      if (typeof form !== "undefined") {
        ...
        dhis2.de.storageManager
          ...
          .fail(function () {
            def.resolve(
              "A form with that ID is not available. Please clear browser cache and try again."
            );
          });
      }
    });
  ...
  this.downloadForm = function (dataSetId, formVersion) {
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        dhis2.de.storageManager.saveFormVersion(
          this.dataSetId,
          this.formVersion
        );
      },
  ...
  this.saveFormVersion = function (dataSetId, formVersion) {
    ...
    try {
      ...
      console.log(
        "Max local storage quota reached, ignored form version: " + dataSetId
      );
    }
  };
  ...
  this.getAllFormVersions = function () {
    return localStorage[KEY_FORM_VERSIONS] != null
      ? JSON.parse(localStorage[KEY_FORM_VERSIONS])
      : null;
  };
  ...
  this.saveDataValue = function (dataValue) {
    var id = this.getDataValueIdentifier(
      dataValue.de,
      dataValue.co,
      dataValue.pe,
      dataValue.ou
    ...
    try {
      ...
      console.log(
        "Max local storage quota reached, not storing data value locally"
      );
    }
  };
  ...
  this.clearDataValueJSON = function (dataValue) {
    this.clearDataValue(dataValue.de, dataValue.co, dataValue.pe, dataValue.ou);
  };
  ...
  this.getAllDataValues = function () {
    return localStorage[KEY_DATAVALUES] != null
      ? JSON.parse(localStorage[KEY_DATAVALUES])
      : null;
  };
  ...
  this.saveCompleteDataSet = function (json) {
    ...
    try {
      ...
      log(
        "Max local storage quota reached, not storing complete registration locally"
      );
    }
  };
    ...
    DAO.store.get("optionSets", uid).done(function (obj) {
      if (obj && obj.optionSet) {
        ...
        if (query == null || query == "" || query == noneVal) {
          options = obj.optionSet.options.slice(
            0,
            dhis2.de.cst.dropDownMaxItems - 1
          );
        } else {
          query = query.toLowerCase();

          for (
            var idx = 0, len = obj.optionSet.options.length;
            idx < len;
            idx++
          ) {
    ...
    if (uids.indexOf(item.uid) == -1) {
      DAO.store.get("optionSets", item.uid).done(function (obj) {
        if (
          !obj ||
          !obj.optionSet ||
          !obj.optionSet.version ||
          !item.v ||
          obj.optionSet.version !== item.v
        ) {
...
dhis2.de.insertOptionSets = function () {
  $(".entryoptionset").each(function (idx, item) {
    ...
    var s2prefix = "s2id_";
    optionSetKey.dataElementId =
      optionSetKey.dataElementId.indexOf(s2prefix) != -1
        ? optionSetKey.dataElementId.substring(
            s2prefix.length,
            optionSetKey.dataElementId.length
          )
        : optionSetKey.dataElementId;

    if (dhis2.de.multiOrganisationUnit) {
      item =
        optionSetKey.organisationUnitId +
        "-" +
        optionSetKey.dataElementId +
        "-" +
        optionSetKey.optionComboId;
    } else {
    ...
    item = item + "-val";
    optionSetKey =
      optionSetKey.dataElementId + "-" + optionSetKey.optionComboId;
...
function dHTApproved() {
  ...
  if ($("#selectedDataSetId").val() == "XV12eKZar28") {
    url =
      "../api/dataValues.json?de=wLKGbYJXIKt&pe=" +
      $("#selectedPeriodId").val() +
      "&ou=" +
      dhis2.de.currentOrganisationUnitId;
  } else if ($("#selectedDataSetId").val() == "wwcxotLHZGY") {
    url =
      "../api/dataValues.json?de=kxPDBQIDg9e&pe=" +
      $("#selectedPeriodId").val() +
      "&ou=" +
      dhis2.de.currentOrganisationUnitId;
  }
...
function sHTApproved() {
  ...
  if ($("#selectedDataSetId").val() == "XV12eKZar28") {
    url =
      "../api/dataValues?de=UjFfCQfJuti&pe=" +
      $("#selectedPeriodId").val() +
      "&ou=" +
      dhis2.de.currentOrganisationUnitId;
  } else if ($("#selectedDataSetId").val() == "wwcxotLHZGY") {
    url =
      "../api/dataValues.json?de=xvkraRAS9AE&pe=" +
      $("#selectedPeriodId").val() +
      "&ou=" +
      dhis2.de.currentOrganisationUnitId;
  }
...
function pMUAccountApproved() {
  ...
  if ($("#selectedDataSetId").val() == "XV12eKZar28") {
    url =
      "../api/dataValues?de=gi6F0xlOeeX&pe=" +
      $("#selectedPeriodId").val() +
      "&ou=" +
      dhis2.de.currentOrganisationUnitId;
  } else if ($("#selectedDataSetId").val() == "wwcxotLHZGY") {
    url =
      "../api/dataValues.json?de=fByaGToLvi9&pe=" +
      $("#selectedPeriodId").val() +
      "&ou=" +
      dhis2.de.currentOrganisationUnitId;
  }
