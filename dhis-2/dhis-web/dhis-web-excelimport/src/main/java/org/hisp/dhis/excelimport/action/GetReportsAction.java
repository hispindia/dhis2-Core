        ...
        try
        {
            ...
            for ( int s = 0; s < totalReports; s++ )
            {
                ...
                if ( reportNode.getNodeType() == Node.ELEMENT_NODE )
                {
                    ...
                    dataSetUid = ((Node) textreportDatasetList.item( 0 )).getNodeValue().trim();

                    DataSet dataSet = dataSetService.getDataSet( dataSetUid );
                    
                    if ( reportType.equals( periodType ) && dataSet.getSources().contains( orgUnit ) )
                    {
                        Report reportObj = new Report( reportId, reportName, reportType, reportModel, reportFileName, dataSetUid );
