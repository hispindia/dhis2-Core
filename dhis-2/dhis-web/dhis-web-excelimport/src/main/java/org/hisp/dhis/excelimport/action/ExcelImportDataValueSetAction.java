...
public class ExcelImportDataValueSetAction implements Action
{
        ...
        throws Exception
    {
        ...
        if( importDate != null && !importDate.equalsIgnoreCase( "" ))
        {
            ...
            // mm/dd/yyyy
            isoPeriod = importDate.split( "/" )[2] + importDate.split( "/" )[0] + importDate.split( "/" )[1];
        }
        else
        {
        ...
        */
        ...
        if( dataImportType != null && !dataImportType.equalsIgnoreCase( "NA" ) )
        {
            ...
            // add for Manipur
            else if( dataImportType.equalsIgnoreCase( "vaccineSessions" ) )
            {
                deCodesImportXMLFileName = "dataValueSetMappingVaccineSessions.xml";
                if( attributeTypeCOCUid != null && !attributeTypeCOCUid.equalsIgnoreCase( "NA" ) )
                {
                    attributeOptionCombo = attributeTypeCOCUid;
                }
