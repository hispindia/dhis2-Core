...
public class ShowedAssociationsDataSetAssignEditorAction implements Action
{
    ...
    */
        ...
        throws Exception
    {
        ...
        {
            grid.addHeader( new GridHeader( dataSet.getDisplayShortName(), dataSet.getId() + "", null, dataSet.getDisplayName(), false, false ) );
        }
        ...
        for ( OrganisationUnit child : children )
        {
            ...
            grid.addRow().addValue( new MetaValue( (int) child.getId(), child.getDisplayName() ) );

            for ( DataSet dataSet : dataSets )
            {
                if ( assignedDataSets.contains( dataSet ) )
                {
                    grid.addValue( new MetaValue( (int) child.getId(), dataSet.getId() + "", "true" ) );
                }
                else
                {
                    grid.addValue( new MetaValue( (int) child.getId(), dataSet.getId() + "", "false" ) );
                }
            }
        }
