...
{	
    ...
    var ouName = xmlObject.getElementsByTagName( "ouName" )[0].firstChild.nodeValue;
...
function getQuarterlyPeriods()
{
	...
	if ( year != "NA" )
	{ 
		  $.post("getQuarterlyPeriods.action",
			{
			  	year : year
			},
			function (data)
			{
				getQuarterlyPeriodsReceived(data);
			},'xml');
	}
	else
	{
...
function getQuarterlyPeriodsReceived( xmlObject )
{	
	var availablePeriods = document.getElementById( "availablePeriods" );
...
function submitImportDataValueForm()
{
		
	var importDValue = document.getElementById( "importDate" ).value;
	
	//alert( "importDValue -- " + importDValue );
	if(importDValue == null || importDValue == "" ) 
	{
