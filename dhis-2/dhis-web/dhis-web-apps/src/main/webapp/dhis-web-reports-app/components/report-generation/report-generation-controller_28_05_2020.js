...
reportsApp.controller('ReportGenerationController',
              ...
              userService,ReportConfigurationService)
    {
        ...
        $scope.selectedOrgUnit = $scope.selOrgUnit[0];
        ...
        //updateReportSetting();
            ...
            $scope.selectedOrgUnitTemp = selection.getSelected();
            $scope.selectedOrgUnit = $scope.selectedOrgUnitTemp[0];
            $scope.selOrgUnit = sessionStorage.getItem('selOrgUnit');
      ...
      // get DHIS reports
        ...
        $scope.reportList = data.reports;
            ...
            angular.forEach($scope.reportList,function(report){
                ...
                console.log( " reportListMapping " + report.id + " -- "+ report.name );
            });

            //console.log( " $scope.reportListMapping " + $scope.reportListMapping  );
    ...
    var updateReportSetting = function(){
        ...
        reportSettingService.getAll().then(function(data){
            ...
            angular.forEach($scope.reportSettings, function(report){
                console.log( " 1 reportListMapping " + report.id );
                report.name = $scope.reportListMapping[report.id].name;
                report.userGroupAccesses = $scope.reportListMapping[report.id].userGroupAccesses;
                //console.log( " 2 reportListMapping " + report.id + " -- "+ report.name + " -- " + report.userGroupAccesses);
                console.log( " 2 Report Name-- " + report.name + "  Report userGroupAccesses-- " + report.userGroupAccesses );
                $scope.reportSettingMapping[report.id] = report;
             ...
             /*
            for(var j=0;j<$scope.reportSettings.length;j++)
            {
                console.log( " 1 reportListMapping " + $scope.reportSettings[j].id );
                $scope.reportSettings[j].name = $scope.reportListMapping[$scope.reportSettings[j].id].name;
                $scope.reportSettings[j].userGroupAccesses = $scope.reportListMapping[$scope.reportSettings[j].id].userGroupAccesses;
                console.log( " 2 reportListMapping " + $scope.reportSettings[j].id + " -- "+ $scope.reportSettings[j].name + " -- " + $scope.reportSettings[j].userGroupAccesses);
                //console.log( " 1 Report Name-- " + report.name + "  Report userGroupAccesses-- " + report.userGroupAccesses );
                $scope.reportSettingMapping[$scope.reportSettings[j].id] = $scope.reportSettings[j];
                angular.copy($scope.reportSettings,$scope.reportSettingsBackup);
            }
            */
    ...
    var isReportInUserGroup = function(userGroupAccesses){
      ...
      angular.forEach(userGroupAccesses,function(userGroupAccess){
          console.log( " 3 userGroupAccess Name-- " + userGroupAccess + " -- " + $scope.userGroupMap[userGroupAccess.userGroupUid] );
