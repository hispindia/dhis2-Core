 ...
 */
public class GetEscalationsListAction extends ActionPagingSupport<ProgramStageInstance>
{
...
//        
        ...
        */
        ...
        {
            /*
            System.out.println( " Escalation start Date " + escalationStartDate.length()  );
            System.out.println( " Escalation end date " + escalationEndDate.length()  );
            System.out.println( " decision start Date " + decisionStartDate.length()  );
            System.out.println( " decision start Date " + decisionEndDate.length()  );
            */
            ...
            if( !"".equals(escalationEndDate) )
            {
                escalationDateCheck = 2;
            }
            
            if( !("".equals(escalationStartDate) && !"".equals(escalationEndDate)) )
            {
            ...
            */
            ...
            if( !(escalationStartDate.equalsIgnoreCase( "" ) && !escalationEndDate.trim().equalsIgnoreCase( "" )) )
            {
                escalationDateCheck = 3;
            }
            */
            ...
            /*    
            if( escalationEndDate == null)
            {
                escalationEndDate = simpleDateFormat.format( new Date() );
            }
            */
            ...
            if( !"".equals(decisionEndDate) )
            {
                decisionDateCheck = 2;
            }
            
            if( !("".equals(decisionStartDate) && !"".equals(decisionEndDate)) )
            {
                decisionDateCheck = 3;
            }
            */
            ...
            if( !decisionEndDate.equalsIgnoreCase( "" ) )
            {
                decisionDateCheck = 2;
            }

            if( !(decisionStartDate.equalsIgnoreCase( "" ) && !decisionEndDate.equalsIgnoreCase( "" )) )
            {
                decisionDateCheck = 3;
            }
            */
            ...
            /*
            if( decisionEndDate == null)
            {
                decisionEndDate = simpleDateFormat.format( new Date() );
            }
            */
            ...
            System.out.println( " Escalation Date Check " + escalationDateCheck  );
            System.out.println( " Establishment Name " + establishmentName  );
            System.out.println( " Establishment Type " + establishmentType  );
            System.out.println( " owner Name " + ownerName  );
            System.out.println( " decision Date Check " + decisionDateCheck  );
            ...
            */
            ...
            if( !escalationStatus.equalsIgnoreCase( "" ) )    // escalation Status filter
                query += "INNER JOIN (" +
                          ...
                          "WHERE dataelementid = 3460 " + // -- escalation Status dataelement id
                          "AND value ILIKE '" + escalationStatus + "' " +
                          ") tdv1 on tdv1.programstageinstanceid = psi.programstageinstanceid ";
           
            if( !establishmentName.equalsIgnoreCase( "" ) )    // establishment Name filter
                query += "INNER JOIN (" +
                         "SELECT trackedentityinstanceid " +
                         "FROM trackedentityattributevalue " +
                         "WHERE trackedentityattributeid = 89 " + //  -- establishment name attribute id
                         "AND value ILIKE '%" + establishmentName + "%' " +
                         ") tav1 on tav1.trackedentityinstanceid = pi.trackedentityinstanceid ";
            
            if( !establishmentType.equalsIgnoreCase( "" ) )   // establishment Type filter
                query += "INNER JOIN (" +
                         "SELECT trackedentityinstanceid " +
                         "FROM trackedentityattributevalue " +
                         "WHERE trackedentityattributeid = 88 " + //  -- establishment type attribute id
            ...
            if( !ownerName.equalsIgnoreCase( "" ) )     // owner Name filter
                query += "INNER JOIN (" +
                         ...
                         "FROM trackedentityattributevalue " +
                         "WHERE trackedentityattributeid = 94 " +  // -- Attribute "Operator/Owner UID"
                         ") ta_UID on ta_UID.trackedentityinstanceid = pi.trackedentityinstanceid " +
                         "INNER JOIN (" +
                         "SELECT op.uid " +            // -- operator uids
                         "FROM trackedentityinstance op " +
                            "INNER JOIN (" +
                            "SELECT trackedentityinstanceid " + // -- select operators with attribute name like '%search pattern%'
                            "FROM trackedentityattributevalue " +
                            "WHERE trackedentityattributeid = 1112 AND value ILIKE '%" + ownerName + "%' " +
                            ") opname ON op.trackedentityinstanceid = opname.trackedentityinstanceid " +
                          "WHERE op.trackedentityid = 104 " +   // -- Operator trackedentity
            ...
            if( !ownerName.equalsIgnoreCase( "" ) )   // owner Name filter
                query += "INNER JOIN (" +
                         "SELECT trackedentityinstanceid " +
                         "FROM trackedentityattributevalue " +
                         "WHERE trackedentityattributeid = 1112 " + //  -- owner Name attribute id
                         "AND value ILIKE '%" + ownerName + "%' " +
            ...
            */
            ...
            /*
            if( !escalationDate.equalsIgnoreCase( "" ) )
                q_where += "AND psi.executiondate = '" + escalationDate + "' ";  // escalation/execution Date filter
            
            if( !decisionDate.equalsIgnoreCase( "" ) )
                q_where += "AND psi.completeddate = '" + decisionDate + "' ";  // decision/complete Date filter
            */
            
            // escalation/execution Date filter
            switch( escalationDateCheck ) 
            {
                case 1: q_where += "AND psi.executiondate >= '"+ escalationStartDate +"' "; break;
                case 2: q_where += "AND psi.executiondate <= '"+ escalationEndDate +"' "; break;
                case 3: q_where += "AND psi.executiondate BETWEEN '" + escalationStartDate + "' AND '" + escalationEndDate +"' "; break;
            }
            // decision/complete Date filter
            switch( decisionDateCheck ) 
            {
                case 1: q_where += "AND psi.completeddate >= '"+ decisionStartDate +"' "; break;
                case 2: q_where += "AND psi.completeddate <= '"+ decisionEndDate +"' "; break;
                case 3: q_where += "AND psi.completeddate BETWEEN '" + decisionStartDate + "' AND '" + decisionEndDate +"' "; break;
            }
...
//        SELECT programstageinstanceid, dataelementid, value from trackedentitydatavalue WHERE dataelementid in (1951,1952,1953,1954,1955);
    ...
    public Map<String, String> getTrackedEntityAttributeValue( String attributeId )
    {
        ...
        try
        {
            ...
            INNER JOIN trackedentityinstance tei ON tei.trackedentityinstanceid = teav.trackedentityinstanceid
            where teav.trackedentityattributeid in (1112 );
            */
