...
import org.hisp.dhis.reports.ReportOrgSpecificDataService;
import org.hisp.dhis.reports.ReportService;
import org.hisp.dhis.reports.Report_in;
import org.hisp.dhis.reports.Report_inDesign;
import org.hisp.dhis.reports.StateDistrictFeedbackReportService;
...
*/
    ...
    public String execute() throws Exception {
        ...
        while (it.hasNext())
        {
            ...
            {
                aggDeMap.putAll(reportService.getResultDataValueFromAggregateTable( (int)currentOrgUnit.getId(), dataElementIdsByComma, aggPeriodIdsByComma));
            }
            ...
            if (aggData.equalsIgnoreCase(USEEXISTINGAGGDATA))
            {
                DeMap.putAll(reportService.getResultDataValueFromAggregateTable( (int)currentOrgUnit.getId(), dataElementIdsByComma, periodIdsByComma));
            }
            else if (aggData.equalsIgnoreCase(GENERATEAGGDATA))
            {
                List<OrganisationUnit> childOrgUnitTree = new ArrayList<OrganisationUnit>(organisationUnitService.getOrganisationUnitWithChildren(currentOrgUnit.getId()));
